...
#### Тип EProductCathegory

Представляет собой перечисление доступных категорий товаров.

#### Тип EPaymentMethod

Представляет собой перечисление доступных способов оплаты заказа.
...
- **catalog:** `IProduct[]` - хранит массив товаров каталога
- **basket:** `string[]` - хранит массив товаров корзины
- **order:** `IOrder | null` - хранит данные о заказе

###### Методы: 

- setCatalog(items: ILot[]) - позволяет установить каталог товаров
- clearCart() - позволяет очистить корзину
...
Отрисовывает универсальное модальное окно с кнопкой сабмита и кнопкой закрытия. Вставляет в кнопку сабмита требуемый текст, выводит внутри любой переданный контент. Конструктор принимает HTML элемент и экземпляр класса `EventEmitter` для инициализации событий.

###### Свойства: 

closeButton: HTMLButtonElement - кнопка закрытия модального окна
content: HTMLElement - наполнения попапа

###### Методы:
- open() - открытие модального окна
- close() - закрытие модального окна
- setButtonText() - установка необходимого текста в кнопку сабмита
...
title: string - название товара
price: number - цена товара
cathegory: EProductCathegory - категория товара
image: string - обложка товара
description?: string - описание товара


### Класс Page

Отвечает за расстановку отображаемых элементов на странице, устанавливает элементы в предназначенные для них контейнеры, работает со счетчиком товаров в корзине (выводит обновленное значение после добавления/удаления товара), позволяет блокировать прокрутку страницы, когда поверх нее вызвано модальное окно.
...
Представляет собой универсальный компонент формы, в котjрый подставляется любой шаблон необходимой формы, работает с отрисовкой формы.

В конструктор принимает шаблон формы и экземпляр EventEmitter для инициализации событий.
...
Предназначен для отрисовки содержимого сообщения об успешно завершенном заказе с итоговой суммой покупки, которое выводится в контент модального окна.

В конструктор принимает элемент, в который будет вставляться сообщение об успешном заказе и функцию, которая будет выполняться при нажатии на кнопку завершения заказа.
###### Свойства:
...
3. Код связывающими слой данных со слоем представления находится в файле index.ts и выполняет роль презентера (проект небольшой, дополнительная сущность не нужна).
...
- `edit-UserDataForm:input` - изменение данных в форме ввода адреса пользователя и выбора способа оплаты
- `edit-UserContactsForm:input` - изменение данных в форме ввода контактных данных пользователя
- `edit-UserDataForm:validation` - событие, сообщающее о необходимости валидации формы ввода адреса пользователя и выбора способа оплаты
- `edit-UserContactsForm:validation` -   событие, сообщающее о необходимости валидации формы ввода контактных данных пользователя
- `edit-UserDataForm:submit` -  сохранение введенных данных и переход к следующему шагу в форме ввода адреса пользователя и выбора способа оплаты
- `edit-UserContactsForm:submit` - сохранение введенных данных и переход к следующему шагу в форме контактных данных пользователя

- `ShoppingCart:select` - вызов отображения корзины в модальном окне

- `ProductCard:select` - выбор товара для отображения в модальном окне
- `ProductCard:add` - добавление товара в корзину
- `ProductCard:delete` - удаление товара из корзины

- `MakeOrder:submit` - подтверждение оформления заказа в модальном окне 
- `Success:submit` - завершение оформления заказа в модальном окне
